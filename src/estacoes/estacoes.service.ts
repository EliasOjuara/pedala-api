...
export class EstacoesService {
    ...
    async criarEstacao(request: EstacaoRequestDto): Promise<void> {
        ...
        if (estacao) {
            throw new BadRequestException(`Já existe uma estação cadastrada com o nome "${request.nome}"`);
        }
    ...
    async buscarEstacaoPorIdESituacao(id: string, situacao: boolean): Promise<EstacaoModel> {
        ...
        if (!estacao) {
            throw new NotFoundException("Nenhuma estação encontrada com este id e situação especificada");
        }
        return estacao;
    }
